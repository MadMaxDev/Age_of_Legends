...
// Transaction_Login
//////////////////////////////////////////////////////////////////////////
LPGamePlay4Web_i::Transaction_Login::Transaction_Login(LPGamePlay4Web_i* pHost)
: m_pHost(pHost)
, m_pPlayer(NULL)
, m_nClientID(0)
, m_nPassLen(0)
...
LPGamePlay4Web_i::Transaction_Login::~Transaction_Login()
{
}
int		LPGamePlay4Web_i::Transaction_Login::DoNext(int nCmd, const void *pData, size_t nDSize)
{
	switch (nCmd)
	{
	case TTY_TC_INIT:
...
int		LPGamePlay4Web_i::Transaction_Login::DoNext_TTY_TC_INIT(const void* pData, size_t nDSize)
{
	GLOGGER2_WRITEFMT(GLOGGER_ID_BILLING, GLGR_STD_HDR(,GP_Transaction_Login)"INIT,%s,0x%X", m_szAccount, m_nClientID);
	if (m_pHost->m_mapTryLoginAccountName2ClientID.has(m_szAccount))
	{
		SendErrRstToClient_And_Done(TTY_LOGIN_RST_LOGININPROCESS, "");
		return 0;
	}
	// 添加尝试登录表中
	m_pHost->m_mapTryLoginAccountName2ClientID.put(m_szAccount, m_nClientID);
	// 设置状态为正常LOGIN中
	m_pPlayer->SetStatus(LPGamePlay4Web_i::PlayerUnit::STATUS_T::LOGINING);

	// 向GS4Web提出正式LOGIN请求
	m_pHost->m_vectrawbuf.resize(wh_offsetof(GS4WEB_DB_ACCOUNT_LOGIN_REQ_T, szPass) + m_nPassLen + 1);
	GS4WEB_DB_ACCOUNT_LOGIN_REQ_T*	pCmd	= (GS4WEB_DB_ACCOUNT_LOGIN_REQ_T*)m_pHost->m_vectrawbuf.getbuf();
	memset(pCmd->nExt, 0, sizeof(pCmd->nExt));
	pCmd->nCmd			= P_GS4WEB_DB_REQ_CMD;
	pCmd->nSubCmd		= DB_ACCOUNT_LOGIN_REQ;
	pCmd->nGroupIdx		= m_pHost->m_cfginfo.nSvrGrpID;
	WH_STRNCPY0(pCmd->szAccount, m_szAccount);
	WH_STRNCPY0(pCmd->szDeviceID, m_szDeviceID);
	memcpy(pCmd->szPass, m_szPassword, m_nPassLen);
	pCmd->szPass[m_nPassLen]	= 0;
	pCmd->nPassLen		= m_nPassLen + 1;
	pCmd->nDeviceType	= m_nDeviceType;
	pCmd->nExt[0]		= m_nTID;
	if (m_pHost->m_msgerGS4Web.SendMsg(m_pHost->m_vectrawbuf.getbuf(), m_pHost->m_vectrawbuf.size()) < 0)
	{
		m_pHost->m_mapTryLoginAccountName2ClientID.erase(m_szAccount);
...
int		LPGamePlay4Web_i::Transaction_Login::DoNext_TTY_TC_GLOBAL_ACCOUNT_LOGIN_RPL(const void* pData, size_t nDSize)
{
	GS4WEB_DB_ACCOUNT_LOGIN_RPL_T*	pRst	= (GS4WEB_DB_ACCOUNT_LOGIN_RPL_T*)pData;
	GLOGGER2_WRITEFMT(GLOGGER_ID_BILLING, GLGR_STD_HDR(,GP_Transaction_Login)"GLOBAL_RPL,0x%"WHINT64PRFX"X,%d", pRst->nAccountID, pRst->nRst);
			...
			if (m_pHost->m_cfginfo.bCreateAccountIfNotExist)
			{
				...
				// 生成新的创建账号transaction
				{
					...
					try
					{
						...
						WH_STRNCPY0(pT->m_szAccountName, m_szAccount);
						memcpy(pT->m_szPass, m_szPassword, sizeof(pT->m_szPass));
...
int		LPGamePlay4Web_i::Transaction_Login::DoNext_TTY_TC_GROUP_ACCOUNT_LOGIN_RPL(const void* pData, size_t nDSize)
{
...
int		LPGamePlay4Web_i::Transaction_Login::DoNext_TTY_TC_CHAR_LOAD_RPL(const void* pData, size_t nDSize)
{
...
int		LPGamePlay4Web_i::Transaction_Login::DoNext_TTY_TC_PLAYER_DROP(const void* pData, size_t nDSize)
{
	GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(,GP_Transaction_Login)"Player Dropped,%s,0x%X", m_szAccount, m_nClientID);
...
int		LPGamePlay4Web_i::Transaction_Login::DealTimeEvent(int nEvent)
{
	switch (nEvent)
	{
	case TE_GLOBAL_LOGIN:
...
int		LPGamePlay4Web_i::Transaction_Login::SendErrRstToClient_And_Done(tty_rst_t nRst, const char* pszPrompt/* = */)
{
	GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(,GP_Transaction_Login)"SendErrRstToClient_And_Done,%s,0x%X,%s,0x%X", pszPrompt, m_nClientID, m_szAccount, (int)nRst);
	// 发送结果
	TTY_LPGAMEPLAY_CLIENT_LOGIN_RST_T	rst;
	rst.nCmd			= TTY_LPGAMEPLAY_CLIENT_LOGIN_RST;
	rst.nRst			= nRst;
...
int		LPGamePlay4Web_i::Transaction_Login::ClearTryLoginPlayer_And_Done()
{
	LPGamePlay4Web_i::PlayerUnit*	pPlayer	= m_pHost->GetPlayerByID(m_nClientID);
	m_pHost->m_mapTryLoginAccountName2ClientID.erase(m_szAccount);
	if (pPlayer != NULL)
...
int		LPGamePlay4Web_i::Transaction_Login::ClearLoginPlayer_And_Done()
{
	LPGamePlay4Web_i::PlayerUnit*	pPlayer	= m_pHost->GetPlayerByID(m_nClientID);
	m_pHost->m_mapAccountID2ClientID.erase(m_nAccountID);
	if (pPlayer != NULL)
