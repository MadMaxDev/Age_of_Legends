...
WHDATAPROP_MAP_BEGIN_AT_ROOT(DBS4Web_i::CFGINFO_T)
	WHDATAPROP_ON_SETVALUE_smp(int, nSvrGrpID, 0)
...
WHDATAPROP_MAP_BEGIN_AT_ROOT(DBS4Web_i::CFGINFO_T::MySQLInfo)
...
int		DBS4Web_i::Detach(bool bQuick)
{
	return 0;
}
...
int		DBS4Web_i::Init(const char *cszCFGName)
{
	m_nTimeNow		= wh_time();
	...
	{
		GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"DBS4Web_i::Init,Init_CFG,%d", iRst);
		return -1;
	}
	...
	{
		GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"DBS4Web_i::Init,%d,m_queueCmdInMT.Init,%d", iRst, m_cfgInfo.nCmdQueueSize);
		return -2;
	}

	if ((iRst=ILogicBase::QueueInit(m_cfgInfo.nCmdQueueSize)) < 0)
	{
		GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"DBS4Web_i::Init,%d,QueueInit,%d", iRst, m_cfgInfo.nCmdQueueSize);
		return -3;
	}
...
// 	if (iRst=(m_queueCmdOut.Init(m_cfgInfo.nCmdQueueSize)) < 0)
// 	{
// 		GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"DBS4Web_i::Init,%d,m_queueCmdOut.Init,%d", iRst, m_cfgInfo.nCmdQueueSize);
// 		return -2;
// 	}
...
int		DBS4Web_i::Init_CFG(const char *cszCFGName)
{
	//分析文件
	WHDATAINI_CMN		ini;
	ini.addobj("DBS4Web", &m_cfgInfo);
	int		iRst		= ini.analyzefile(cszCFGName);
	if (iRst < 0)
	{
		GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"DBS4Web_i::Init_CFG,%d,ini.analyzefile,%s", iRst, ini.printerrreport());
		return -1;
	}

	return 0;
}
int		DBS4Web_i::Init_DB()
{
	...
	q.SpawnQuery("call game_init_db(%d,%d,%s)", m_cfgInfo.nSvrGrpID, SVRGRP_DBS_IDX, SP_RESULT);
	int	nPreRet		= 0;
	...
	q.FreeResult();
	q.SpawnQuery("call game_init_fixhour(%s)", SP_RESULT);
	q.ExecuteSPWithResult(nPreRet);
	if (nPreRet != MYSQL_QUERY_NORMAL)
	{
		return -13;
	...
	q.FreeResult();
	q.SpawnQuery("call game_init_worldgoldmine(%s)", SP_RESULT);
	q.ExecuteSPWithResult(nPreRet);
	if (nPreRet != MYSQL_QUERY_NORMAL)
	{
		return -23;
...
int		DBS4Web_i::Init_CFG_Reload(const char *cszCFGName)
{
	//分析文件
	WHDATAINI_CMN		ini;
	ini.addobj("DBS4Web", &m_cfgInfo);
	...
	if (iRst < 0)
	{
		GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"DBS4Web_i::Init_CFG_Reload,%d,ini.analyzefile,%s", iRst, ini.printerrreport());
		return -1;
	}
...
int		DBS4Web_i::Release()
{
	...
	for (size_t i=0; i<m_vectFreeMySQLHandle.size(); i++)
	{
		mysql_close(m_vectFreeMySQLHandle[i]);
	}
	GLOGGER2_WRITEFMT(GLOGGER_ID_CMN, GLGR_STD_HDR(1359,DBS4Web_RT)"Main thread release, m_mapTid2MySQLHandle.size():%d,m_nCurMySQLHandleNum:%d,m_vectFreeMySQLHandle.size():%d", 
...
int		DBS4Web_i::SureSend()
{
	return 0;
}

int		DBS4Web_i::Tick_BeforeDealCmdIn()
{
...
int		DBS4Web_i::Tick_AfterDealCmdIn()
{
	if (m_bStopping)
	{	
		if ((!m_cfgInfo.bNeedAllCntrQuitThenStop || m_nLPCntrID!=-1) && m_queueCmdInMT.GetSize()==0 && m_nMood!=CMN::ILogic::MOOD_STOPPED)
		{
			m_nMood				= CMN::ILogic::MOOD_STOPPED;
			GLOGGER2_WRITEFMT(GLOGGER_ID_CMN, GLGR_STD_HDR(1359,DBS4Web_RT)"DBS4Web_i::DealCmdIn_One,end now,t1:%d,t2:%d", m_tickStoppingBegin, wh_gettickcount());
		}
	}
	return 0;
}

int		DBS4Web_i::SendMsg_MT(int iCntrID, const void *pData, int nDSize)
{	
...
int		DBS4Web_i::RecvMsg_MT(whvector<char>& vectBuf)
{
	m_queueCmdInMT.lock();
...
void	DBS4Web_i::CreateWorkerThread()
{
//	whtid_t			tid;
	...
	if (whthread_create(&pInfo->tid, _DBS4Web_Worker_ThreadProc, pInfo) < 0)
	{
...
void	DBS4Web_i::CheckThreadActive()
{
	whvector<THREAD_INFO_T>	vectTmp;
	for (size_t i=0; i<m_vectThreadInfo.size(); i++)
	{
		...
		if (whthread_isend(tid))		//线程已经失效了
		{
			...
			GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1359,DBS4Web_RT)"thread not active,thread id:0x%X, status:%d", tid, m_nMood);
		}
	}

	for (size_t i=0; i<vectTmp.size(); i++)
	{
...
MYSQL*	DBS4Web_i::GetSelfMySQLHandle(whtid_t tid)
{
...
MYSQL*	DBS4Web_i::BorrowMySQLHandle(whtid_t tid)
{
	whlockmutex		tmpLock(&m_lockMySQLHandleOp);	//会自动加锁或者释放
	MYSQL*	pMySQL	= NULL;
...
void	DBS4Web_i::ReturnMySQLHandle(whtid_t tid, MYSQL* pMYSQL)
{
	whlockmutex		tmpLock(&m_lockMySQLHandleOp);	//会自动加锁或者释放
...
MYSQL*	DBS4Web_i::GenMySQLHandle()
{
...
void*	DBS4Web_i::_DBS4Web_Worker_ThreadProc(void* pParam)
{
	whvector<char>	vectMsgBuf;
	whvector<char>	vectRawBuf;		//用于底层组装指令的
	vectMsgBuf.reserve(4*1024);		//先预留4k
	vectRawBuf.reserve(2*1024*1024);		//先预留2M
	THREAD_INFO_T*	pThreadInfo		= (THREAD_INFO_T*)pParam;
	DBS4Web_i*	pHost				= pThreadInfo->pHost;			// pHost一定是有效的,在创建线程之前已经赋值好了
	...
	}
	{
		...
		GLOGGER2_WRITEFMT(GLOGGER_ID_CMN, GLGR_STD_HDR(1359,DBS4Web_RT)"thread start to work,threadID:0x%X,status:%d,pThreadInfo:0x%X,host:0x%X", tid, pHost->m_nMood, pThreadInfo, pHost);
	}
	...
	{
		whlockmutex		tmpLock(&pHost->m_lockWriteLog);
		GLOGGER2_WRITEFMT(GLOGGER_ID_CMN, GLGR_STD_HDR(1359,DBS4Web_RT)"thread end normal,threadID:0x%X,status:%d,pThreadInfo:0x%X,host:0x%X", tid, pHost->m_nMood, pThreadInfo, pHost);
	}
	return NULL;
}

int		DBS4Web_i::InitGenAllSQLHandle()
{
	int		iNum	= 0;
	for (int i=0; i<m_cfgInfo.nThreadNum; i++)
	{
		...
		{
			GLOGGER2_WRITEFMT(GLOGGER_ID_ERROR, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"%s,,gen sql handle failed", __FUNCTION__);
		}
	}
	GLOGGER2_WRITEFMT(GLOGGER_ID_CMN, GLGR_STD_HDR(1355,DBS4Web_INT_RLS)"%s,,gen sql handle,num:%d", __FUNCTION__, iNum);
