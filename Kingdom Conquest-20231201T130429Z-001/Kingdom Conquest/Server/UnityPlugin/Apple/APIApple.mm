...
static int znum[6] = {60,300,680,1280,3280,6480};
- (id)init
{
    if( (self=[super init])) {
        // 初始化IAP
        ...
        for(int i=0;i<6;i++) {
            ...
            [vals addObject:[NSString stringWithFormat:@"app.pocketking.diamond.%d", znum[i]]];
        }
...
- (void)buy:(int)num accountid:(NSString *)accid serial:(NSString *)cooOrderSerial
{
    ...
    if( !g_cur_id ) {
        g_cur_id    = @"app.pocketking.diamond.60";
    }
...
- (void)getUUID
{
    ...
    if( uuid ) {
        [self mycallbacktounity:[NSString stringWithFormat:@"uuid %@", uuid]];
    }
    else {
...
extern "C" {
    ...
    int cmd_apple(const char *cmd, const char *param) {
        ...
        else if( strcmp(cmd, "getUUID")==0 ) {
            [delegateObject getUUID];
        }
        else {
