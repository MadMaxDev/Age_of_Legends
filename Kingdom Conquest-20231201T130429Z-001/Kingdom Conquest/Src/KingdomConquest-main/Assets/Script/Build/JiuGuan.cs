...
using CMNCMD;
using STCCMD;
public class JiuGuan : MonoBehaviour {
	//建筑状态 正常、建造、升级 
	...
	//根据服务器发来的信息初始化酒馆   
	void InitialJiuGuan(uint level)
	{
	...
	//打开升级酒馆窗口  
	void OpenJiuGuanInfoWin()
	{
		JiuGuanInfoWin.JiuGuanUnit unit;
		unit.level = NowLevel;
		unit.build_state = BuildState;
		unit.build_end_time = BuildEndTimeSec;
		GameObject infowin = U3dCmn.GetObjFromPrefab("JiuGuanInfoWin");
		if(infowin != null)
		{
	...
	void UpgradeBeginCountdown()
	{
		...
		{
			BuildInfo info = (BuildInfo)U3dCmn.GetBuildingInfoFromMb((int)BuildingSort.JIUGUAN,(int)(NowLevel+1));
