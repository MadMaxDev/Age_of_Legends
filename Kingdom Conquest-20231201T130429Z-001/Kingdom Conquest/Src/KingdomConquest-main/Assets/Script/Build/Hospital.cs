...
public class Hospital : MonoBehaviour {
	...
	//根据服务器发来的信息初始化医馆     
	void InitialHospital(uint level)
	{
	...
	//打开升级医馆窗口   
	void OpenHospitalWin()
	{
		HospitalWin.HospitalUnit unit;
		unit.level = NowLevel;
		unit.build_state = BuildState;
		unit.build_end_time = BuildEndTimeSec;
		GameObject infowin = U3dCmn.GetObjFromPrefab("HospitalWin");
		if(infowin != null)
		{
	...
	void UpgradeBeginCountdown()
	{
		...
		{
			BuildInfo info = (BuildInfo)U3dCmn.GetBuildingInfoFromMb((int)BuildingSort.YIGUAN,(int)(NowLevel+1));
