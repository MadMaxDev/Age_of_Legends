...
using CMNCMD;
using STCCMD;
public class TrainGround : MonoBehaviour {
	//建筑状态 正常、建造、升级 
	...
	//根据服务器发来的信息初始化训练场     
	void InitialTrainGround(uint level)
	{
	...
	//打开升级训练场窗口   
	void OpenTrainGroundWin()
	{
		TrainGroundWin.TrainGroundUnit unit;
		unit.level = NowLevel;
		unit.build_state = BuildState;
		unit.build_end_time = BuildEndTimeSec;
		GameObject infowin = U3dCmn.GetObjFromPrefab("TrainGroundWin");
		if(infowin != null)
		{
	...
	void UpgradeBeginCountdown()
	{
		...
		{
			BuildInfo info = (BuildInfo)U3dCmn.GetBuildingInfoFromMb((int)BuildingSort.XIULIAN,(int)(NowLevel+1));
