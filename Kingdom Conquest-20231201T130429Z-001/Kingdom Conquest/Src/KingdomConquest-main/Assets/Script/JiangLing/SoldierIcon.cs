...
using STCCMD;
//当前可用士兵的ICON单元脚本  
public class SoldierIcon : PagingStorageSlot {
	int mItemID = -1;
	...
	void InitialSoldierInfo( )
	{
		...
		if(gid>=0 && gid<WithSoldierWin.soldier_key_array.Count)
		{
			
			//ArrayList soldier_key_array = new ArrayList(SoldierManager.SoldierMap.Keys);
			WithSoldierWin.soldier_key_array.Sort();
			nowinfo =  (SoldierUnit)SoldierManager.SoldierMap[(string)WithSoldierWin.soldier_key_array[gid]];
			...
			int soldier_num = 0;
			if(WithSoldierWin.select_hero.nArmyType9==nowinfo.nExcelID1 && WithSoldierWin.select_hero.nArmyLevel10 == nowinfo.nLevel2)
			{
				soldier_num = (int)nowinfo.nNum3+ WithSoldierWin.select_hero.nArmyNum11;
			}
			else 
			{
			...
			if(WithSoldierWin.select_hero.nArmyType9 >0 && WithSoldierWin.select_hero.nArmyLevel10 >0)
			{
				if(WithSoldierWin.select_hero.nArmyType9==nowinfo.nExcelID1 && WithSoldierWin.select_hero.nArmyLevel10 == nowinfo.nLevel2)
				{
					U3dCmn.SendMessage("WithSoldierWin","ShowDetail",gameObject);
					SetSelect(true);
				}
			}
			else if(gid == 0)
